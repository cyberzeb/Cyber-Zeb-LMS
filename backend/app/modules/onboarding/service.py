...
def _institution_link(slug: str) -> str:
    domain = settings.PUBLIC_BASE_DOMAIN.strip().removeprefix("https://").removeprefix("http://").rstrip("/")
...
def serialize_service_request(
    sr: ServiceRequest, tenant: Tenant | None = None
) -> ServiceRequestOut:
...
def serialize_addon_request(req: AddOnModuleRequest, tenant: Tenant) -> AddOnModuleRequestOut:
...
class OnboardingService:
    ...
    async def ensure_default_module_catalog(self) -> None:
        ...
        for key in ModuleKey:
            existing = await self.repo.get_module_catalog_by_key(key.value)
            if existing:
                continue
            self.db.add(
                ModuleCatalogItem(
    ...
    async def create_service_request(
        ...
        *,
        ...
        if existing:
            ...
            return serialize_service_request(existing, tenant)
        ...
        # Internal alert — failures must not roll back the request.
        ...
        notify_to = settings.SUPER_ADMIN_NOTIFY_EMAIL or settings.GMAIL_USER
        if notify_to:
        ...
        return serialize_service_request(reloaded or sr)
    ...
    async def create_addon_request(
        ...
        *,
        ...
        if existing:
            ...
            return serialize_addon_request(existing, tenant)
        ...
        await self.db.commit()
        return serialize_addon_request(req, tenant)
    ...
    async def list_addon_requests(
        ...
        for item in items:
            ...
            if tenant:
                out.append(serialize_addon_request(item, tenant))
        return out, total
    ...
    async def list_service_requests(
        ...
        for sr in items:
            ...
            out.append(serialize_service_request(sr, tenant))
        return out, total
    ...
    async def get_service_request(self, request_id: uuid.UUID) -> ServiceRequestOut:
        ...
        tenant = await self.repo.get_tenant_by_service_request(sr.id)
        return serialize_service_request(sr, tenant)
    ...
    async def send_invoice(
        ...
        *,
        ...
        sr.invoice_amount = body.invoice_amount
        sr.invoice_currency = body.invoice_currency.upper()
        ...
        *,
        ...
        if sr.status == ServiceRequestStatus.ACTIVATED and existing_tenant:
            return ActivateResponse(
                service_request=serialize_service_request(sr, existing_tenant),
        ...
        return ActivateResponse(
            service_request=serialize_service_request(reloaded or sr, tenant),
            tenant=tenant_out,
    ...
    async def send_addon_invoice(
        ...
        *,
        ...
        req.invoice_amount = body.invoice_amount
        req.invoice_currency = body.invoice_currency.upper()
    ...
    async def list_renewals(self, *, days: int | None = 30) -> list[RenewalTenantOut]:
        ...
        return [
            RenewalTenantOut(
                ...
                institution_link=_institution_link(t.slug or t.code),
            )
            for t in tenants
        ]
    ...
    async def mark_tenant_renewed(
        ...
        *,
        ...
        await self.db.commit()
        return (await self.list_renewals(days=None))[0] if False else RenewalTenantOut(
            id=tenant.id,
            name=tenant.name,
            slug=tenant.slug or tenant.code,
            ...
            renewal_date=tenant.renewal_date,
            institution_link=_institution_link(tenant.slug or tenant.code),
        )
