...
import { MessageSquare, MessageSquareText, Pin, Users } from 'lucide-react'
...
const tabs = ['All threads', 'Pinned', 'My courses']
...
export function StudentForumPage() {
  ...
  const stats = useMemo(() => {
    if (!data) return { threads: 0, pinned: 0, replies: 0 }
    return {
      threads: data.forumThreads.length,
      pinned: data.forumThreads.filter((t) => t.pinned).length,
      replies: data.forumThreads.reduce((sum, t) => sum + t.replies, 0),
    }
  }, [data])
  ...
  if (isError || !data) return <StudentPageError message="Failed to load discussion forum." />

  return (
    <div className="flex flex-col gap-6 md:gap-8">
      <PageHeader
        title="Discussion Forum"
        subtitle="Collaborate with classmates, ask questions, and share study resources."
        actions={
      ...
      <div className="flex flex-col gap-3">
        ...
        {filtered.length === 0 ? (
          <GlassCard className="p-10 text-center">
            <MessageSquare size={32} className="mx-auto text-navy-300 mb-3" />
            <p className="text-[14px] font-semibold text-navy-900">No threads yet</p>
            <p className="text-[12.5px] text-secondary-text mt-1">Start a conversation with your classmates.</p>
          </GlassCard>
        ) : null}
      </div>
    </div>
