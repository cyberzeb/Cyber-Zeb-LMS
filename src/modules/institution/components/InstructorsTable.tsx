...
import { instructorAssignmentLabel } from '../utils/courseAssignmentUtils'
...
interface InstructorsTableProps {
  ...
  onEdit: (instructor: PersonRow) => void
  onDelete: (instructor: PersonRow) => void
}
    ...
    <GlassCard className="p-0 overflow-hidden">
      ...
      <div className="divide-y divide-divider/50">
        {instructors.map((instructor, i) => {
          const status = statusMap[instructor.status]
          const teaching = instructorAssignmentLabel(courses, instructor.id, instructor.name)
          return (
            <div
              key={instructor.id}
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[11.5px] text-secondary-text truncate">{instructor.email}</div>
                </div>
              </div>

              <div>
                <span className="inline-block text-[10.5px] font-bold px-2.5 py-1 rounded-md bg-navy-50 text-navy-700">
                  {campusLabel(instructor.campusId, campuses)}
                </span>
              </div>

              <div className="text-[12.5px] text-navy-700 truncate" title={teaching}>
                {teaching}
              </div>
              <div className="text-[12px] text-secondary-text">{instructor.lastActive}</div>
