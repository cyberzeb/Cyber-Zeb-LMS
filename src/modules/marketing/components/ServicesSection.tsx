        ...
        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {SERVICES.map((s, index) => {
            ...
            return (
              <div
                key={s.title}
                className="group bg-white/[0.06] border border-white/10 rounded-2xl p-6 hover:bg-white/[0.09] hover:border-lemon-500/40 transition-all duration-300 hover:-translate-y-1 animate-fade-in-up"
                style={{ animationDelay: `${index * 0.07}s` }}
              >
                <div className="w-11 h-11 rounded-xl bg-lemon-500/15 flex items-center justify-center text-lemon-500 mb-4 group-hover:bg-lemon-500 group-hover:text-navy-900 group-hover:scale-110 transition-all duration-300">
